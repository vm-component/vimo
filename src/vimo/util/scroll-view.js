...
//       // 当再次出发scroll事件时, 清除之前的计时器, 另外: App部分设置scrollEnd的超时时间为100ms
...
export class ScrollView {
  ...
  init (ele, contentTop, contentBottom) {
    if (!this.initialized) {
      ...
      if (this._js) {
        ...
        // console.debug('contentBottom:'+contentBottom)
  ...
  enableJsScroll (contentTop, contentBottom) {
    ...
    // 滚动的最大距离, 即scrollHeight
    function setMax () {
      if (!max) {
        // ******** DOM READ ****************
        // max = ele.scrollHeight;
        max = ele.scrollHeight + contentTop - contentBottom + 16;
      }
    }
